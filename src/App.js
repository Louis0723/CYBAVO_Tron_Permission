...
function Type(props) {
  ...
  const addresses = useMemo(() => {
    ...
    for (let i = 0; i < addressCount; i++) {
      result.push(
        <div key={i}>
          <input type="text" className="rounded w-8/12" placeholder={`${typeName} address`}
            onChange={(e) => {
              typeResult[i] = {
          ...
          <input type="number" className="rounded w-4/12" placeholder={`${typeName} weight`}
            onChange={(e) => {
              typeResult[i] = {
...
function Active(props) {
  ...
  return (
    <div className='inline-block shadow-xl ring-1 ring-gray-900/5  sm:p-10 sm:m-5'>
